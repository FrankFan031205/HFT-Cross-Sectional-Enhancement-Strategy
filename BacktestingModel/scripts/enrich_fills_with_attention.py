...
def main():
    ...
    fill_path = "outputs/fills/fills_touched_mlp2_h60_202410_100.csv"
    quote_path = cfg["input"]["quote_decision_path"]
    out_path = "outputs/fills/fills_touched_attention_h60_202410_100.csv"
    ...
    if signal_col not in quotes.columns:
        candidates = [c for c in quotes.columns if "attention" in c.lower() or "hidden" in c.lower() or "factor" in c.lower() or "pred" in c.lower()]
        raise RuntimeError(
            f"{signal_col} not found in quote decision file. Candidate signal columns: {candidates}"
        )

    if "datetime" not in quotes.columns:
        raise RuntimeError("quote decision file missing datetime")

    if "securityid" not in quotes.columns:
        raise RuntimeError("quote decision file missing securityid")
    ...
    for c in ["pred_ret", "fair_mid", "bid_edge", "ask_edge", "risk_state"]:
