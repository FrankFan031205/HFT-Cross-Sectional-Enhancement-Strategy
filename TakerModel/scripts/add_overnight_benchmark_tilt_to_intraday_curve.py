...
def pick_col(df, candidates, name, required=True):
    for c in candidates:
        if c in df.columns:
            return c
    if required:
        raise KeyError(f"cannot find {name}: {candidates}; columns={df.columns.tolist()}")
    return None
...
def norm_sid(df):
    ...
    if "sid" in x.columns:
        ...
        raise KeyError(f"no sid/securityid columns: {x.columns.tolist()}")
    return x


def ts_to_dt(date_int, ts):
...
def position_weights(pos_snap, capital):
    x = norm_sid(pos_snap)

    if "mid_price" in x.columns and "actual_shares_after" in x.columns:
        x["weight"] = (
            pd.to_numeric(x["actual_shares_after"], errors="coerce").fillna(0.0)
            * pd.to_numeric(x["mid_price"], errors="coerce").fillna(0.0)
            / float(capital)
        )
    else:
        w_col = pick_col(
        ...
        x["weight"] = pd.to_numeric(x[w_col], errors="coerce").fillna(0.0)

    out = (
        x[["sid_norm", "weight"]]
        .dropna()
        .rename(columns={"sid_norm": "sid"})
        .groupby("sid", as_index=False)["weight"]
        .sum()
    )
    return out


def benchmark_weights(pos_snap):
    x = norm_sid(pos_snap)
    if "benchmark_weight" not in x.columns:
        raise KeyError("positions need benchmark_weight. Use enriched positions file.")
...
def get_snapshot(pos, date_int, dt=None, mode="le"):
    x = pos[pos["date_int"] == int(date_int)].copy()
    if x.empty:
        return x

    if dt is None:
        snap_dt = x["datetime"].min()
        return x[x["datetime"] == snap_dt].copy()

    if mode == "le":
        cand = x[x["datetime"] <= dt]
        if cand.empty:
            cand = x
        ...
        if cand.empty:
            cand = x
        snap_dt = cand["datetime"].min()

    return cand[cand["datetime"] == snap_dt].copy()


def make_benchmark_tilt(universe, active_oneway, top_frac, bottom_frac, pred_col):
    """
    target = benchmark_weight + active tilt.
    active_oneway = buy amount = sell amount.
    long-only, fully invested, benchmark-relative.
    """
    x = universe.copy()
    ...
    x["target_weight"] = x["benchmark_weight"]
    x["active_buy_weight"] = 0.0
    x["active_sell_weight"] = 0.0

    top = x.sort_values(pred_col, ascending=False).head(top_n).copy()
    bottom = x[x["benchmark_weight"] > 1e-12].sort_values(pred_col, ascending=True).head(bottom_n).copy()

    sell_budget = min(float(active_oneway), float(bottom["benchmark_weight"].sum()))
    if sell_budget <= 1e-12:
        return x

    sell = bottom[["sid", "benchmark_weight"]].copy()
    sell["active_sell_new"] = sell_budget * sell["benchmark_weight"] / sell["benchmark_weight"].sum()

    buy = top[["sid"]].copy()
    buy["active_buy_new"] = sell_budget / len(buy)

    x = x.merge(sell[["sid", "active_sell_new"]], on="sid", how="left")
    x = x.merge(buy[["sid", "active_buy_new"]], on="sid", how="left")

    x["active_sell_new"] = x["active_sell_new"].fillna(0.0)
    x["active_buy_new"] = x["active_buy_new"].fillna(0.0)

    x["active_sell_weight"] = x["active_sell_new"]
    x["active_buy_weight"] = x["active_buy_new"]
    x["target_weight"] = x["benchmark_weight"] - x["active_sell_weight"] + x["active_buy_weight"]
    x["target_weight"] = x["target_weight"].clip(lower=0.0)

    # 数值上重新归一到 benchmark gross
    bg = float(x["benchmark_weight"].sum())
    tg = float(x["target_weight"].sum())
    if tg > 1e-12:
        x["target_weight"] = x["target_weight"] * bg / tg

    return x.drop(columns=["active_sell_new", "active_buy_new"], errors="ignore")


def l1_turnover(a, b):
    x = a.merge(b, on="sid", how="outer").fillna(0.0)
    return float((x["w_a"] - x["w_b"]).abs().sum())
...
def main():
    ...
    ap.add_argument("--entry-ts", type=int, default=14210)
    ap.add_argument("--active-oneway", type=float, default=0.05)
    ...
    if "date" in pos.columns:
        pos["date_int"] = (
            pos["date"].astype(str).str.replace(r"\D", "", regex=True).str.slice(0, 8).astype(int)
        )
    else:
    ...
    if args.pred_col not in ov.columns:
        raise KeyError(f"missing pred col {args.pred_col}: {ov.columns.tolist()}")
    if args.ret_col not in ov.columns:
        raise KeyError(f"missing ret col {args.ret_col}: {ov.columns.tolist()}")
    ...
    ov[args.ret_col] = pd.to_numeric(ov[args.ret_col], errors="coerce").fillna(0.0)
    ...
    for d in sorted(ov["date_int"].dropna().unique()):
        ...
        entry_dt = ts_to_dt(d, args.entry_ts)
        snap = get_snapshot(pos, d, dt=entry_dt, mode="le")
        if snap.empty:
            continue

        next_snap = get_snapshot(pos, nd, dt=None, mode="first")
        if next_snap.empty:
            continue

        base_w = position_weights(snap, args.capital).rename(columns={"weight": "base_weight"})
        next_w = position_weights(next_snap, args.capital).rename(columns={"weight": "next_weight"})
        bw = benchmark_weights(snap)

        z = ov[ov["date_int"] == d][["sid", args.pred_col, args.ret_col]].copy()
        z = z.rename(columns={args.ret_col: "overnight_ret"})

        x = z.merge(bw, on="sid", how="left")
        x = x.merge(base_w, on="sid", how="left")
        x = x.merge(next_w, on="sid", how="left")
        x["benchmark_weight"] = x["benchmark_weight"].fillna(0.0)
        x["base_weight"] = x["base_weight"].fillna(0.0)
        x["next_weight"] = x["next_weight"].fillna(0.0)

        x = make_benchmark_tilt(
            x,
            active_oneway=float(args.active_oneway),
            top_frac=float(args.top_frac),
            bottom_frac=float(args.bottom_frac),
            pred_col=args.pred_col,
        )

        entry_turn = l1_turnover(
            x[["sid", "base_weight"]].rename(columns={"base_weight": "w_a"}),
            x[["sid", "target_weight"]].rename(columns={"target_weight": "w_b"}),
        )
        exit_turn = l1_turnover(
            x[["sid", "target_weight"]].rename(columns={"target_weight": "w_a"}),
            x[["sid", "next_weight"]].rename(columns={"next_weight": "w_b"}),
        )
        total_turn = entry_turn + exit_turn
        cost_ret = total_turn * float(args.stock_cost_bps) / 10000.0

        base_ov = float((x["base_weight"] * x["overnight_ret"]).sum())
        target_ov_gross = float((x["target_weight"] * x["overnight_ret"]).sum())
        target_ov_net = target_ov_gross - cost_ret
        bench_ov = float((x["benchmark_weight"] * x["overnight_ret"]).sum())
        ...
        daily_rows.append({
            ...
            "target_overnight_ret_gross": target_ov_gross,
            "strategy_overnight_ret": target_ov_net,
            "benchmark_overnight_ret": bench_ov,
            "overnight_alpha_ret": target_ov_net - bench_ov,
            "base_gross": float(x["base_weight"].sum()),
            "target_gross": float(x["target_weight"].sum()),
            "next_gross": float(x["next_weight"].sum()),
            "benchmark_gross": float(x["benchmark_weight"].sum()),
            "active_buy_weight": float(x["active_buy_weight"].sum()),
            "active_sell_weight": float(x["active_sell_weight"].sum()),
            "entry_turnover": entry_turn,
            "exit_turnover": exit_turn,
    ...
    if daily.empty:
        raise RuntimeError("no daily overnight rows built")
    ...
    summary = pd.DataFrame([{
        ...
        "avg_target_overnight_ret_gross": float(daily["target_overnight_ret_gross"].mean()),
        ...
        "avg_next_gross": float(daily["next_gross"].mean()),
        "avg_benchmark_gross": float(daily["benchmark_gross"].mean()),
        "avg_active_buy_weight": float(daily["active_buy_weight"].mean()),
        "avg_active_sell_weight": float(daily["active_sell_weight"].mean()),
        "avg_entry_turnover": float(daily["entry_turnover"].mean()),
        "avg_exit_turnover": float(daily["exit_turnover"].mean()),
        "avg_total_turnover": float(daily["total_turnover"].mean()),
        "avg_cost_ret": float(daily["cost_ret"].mean()),
    ...
    ax.set_title("V16 Intraday + Benchmark-Relative Overnight Target")
