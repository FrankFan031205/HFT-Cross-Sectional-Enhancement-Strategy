...
def main():
    ...
    # v6: if yesterday loses more than this threshold, block new buys for cooldown_days
    ap.add_argument("--daily_loss_stop", type=float, default=-1000000.0)
    ap.add_argument("--cooldown_days", type=int, default=1)
    ap.add_argument("--risk_off_target_scale", type=float, default=0.0)

    # v8: intraday market regime filter
    ap.add_argument("--market_regime_ret_stop", type=float, default=-0.02)
    ap.add_argument("--market_regime_up_ratio_stop", type=float, default=0.25)
    ap.add_argument("--market_regime_target_scale", type=float, default=0.3)
    ap.add_argument("--market_regime_min_minutes", type=int, default=30)
    ...
    total_blocked_loss_stop = 0
    total_risk_off_minutes = 0
    total_market_risk_off_minutes = 0
    total_blocked_market_regime = 0

    current_date = None
    day_start_equity = capital
    cooldown_left_days = 0
    block_buy_today = False
    day_open_mid = {}
    day_minute_count = 0
    ...
    for dt, g in df.groupby("execution_datetime", sort=True):
        ...
        if current_date is None:
            ...
            prev_day_pnl = prev_equity - day_start_equity

            if prev_day_pnl < args.daily_loss_stop:
                cooldown_left_days = max(cooldown_left_days, int(args.cooldown_days))
            ...
            block_buy_today = cooldown_left_days > 0
            if cooldown_left_days > 0:
                cooldown_left_days -= 1
        ...
        for _, r in g.iterrows():
            ...
            if np.isfinite(mid) and mid > 0:
                if sid not in day_open_mid:
                    day_open_mid[sid] = mid
                last_mid[sid] = mid

        intraday_rets = []
        for sid0, open_mid0 in day_open_mid.items():
            cur_mid0 = last_mid.get(sid0, np.nan)
            if np.isfinite(open_mid0) and open_mid0 > 0 and np.isfinite(cur_mid0) and cur_mid0 > 0:
                intraday_rets.append(cur_mid0 / open_mid0 - 1.0)

        if len(intraday_rets) > 0:
            market_ret_mean_now = float(np.mean(intraday_rets))
            market_up_ratio_now = float(np.mean(np.asarray(intraday_rets) > 0))
        else:
            market_ret_mean_now = np.nan
            market_up_ratio_now = np.nan

        market_risk_off = (
            day_minute_count >= int(args.market_regime_min_minutes)
            and (
                (np.isfinite(market_ret_mean_now) and market_ret_mean_now < args.market_regime_ret_stop)
                or (np.isfinite(market_up_ratio_now) and market_up_ratio_now < args.market_regime_up_ratio_stop)
            )
        )
        ...
        zero_target_exit = 0
        blocked_loss_stop = 0
        blocked_market_regime = 0
        risk_off_active = int(block_buy_today)
        market_risk_off_active = int(market_risk_off)

        g2 = g.copy()

        # v7: when risk-off is active, shrink target weight directly.
        # scale=0 means force de-risk / exit according to target_qty logic.
        if block_buy_today:
            g2["_target_weight"] = g2["_target_weight"].fillna(0.0) * float(args.risk_off_target_scale)

        if market_risk_off:
            g2["_target_weight"] = g2["_target_weight"].fillna(0.0) * float(args.market_regime_target_scale)
        ...
        total_blocked_loss_stop += blocked_loss_stop
        total_blocked_market_regime += blocked_market_regime
        total_risk_off_minutes += risk_off_active
        total_market_risk_off_minutes += market_risk_off_active
        ...
        minute_rows.append({
            ...
            "blocked_loss_stop": blocked_loss_stop,
            "blocked_market_regime": blocked_market_regime,
            "risk_off_active": risk_off_active,
            "market_risk_off_active": market_risk_off_active,
            "market_ret_mean_now": market_ret_mean_now,
            "market_up_ratio_now": market_up_ratio_now,
    ...
    daily = (
        ...
        .agg(
            ...
            blocked_loss_stop=("blocked_loss_stop", "sum"),
            blocked_market_regime=("blocked_market_regime", "sum"),
            risk_off_minutes=("risk_off_active", "sum"),
            market_risk_off_minutes=("market_risk_off_active", "sum"),
            market_ret_mean_last=("market_ret_mean_now", "last"),
            market_up_ratio_last=("market_up_ratio_now", "last"),
    ...
    summary = {
        ...
        "total_blocked_loss_stop": int(total_blocked_loss_stop),
        "total_blocked_market_regime": int(total_blocked_market_regime),
        "total_risk_off_minutes": int(total_risk_off_minutes),
        "total_market_risk_off_minutes": int(total_market_risk_off_minutes),
        "market_regime_ret_stop": args.market_regime_ret_stop,
        "market_regime_up_ratio_stop": args.market_regime_up_ratio_stop,
        "market_regime_target_scale": args.market_regime_target_scale,
        "market_regime_min_minutes": args.market_regime_min_minutes,
        "daily_loss_stop": args.daily_loss_stop,
        "cooldown_days": args.cooldown_days,
        "risk_off_target_scale": args.risk_off_target_scale,
