...
def fwz_f12_bias_spread_reversal():
    fac = -_c("BIAS5") * _rel_spread()
    return fac.alias("fwz_f12_bias_spread_reversal")
...
# =========================================================
# fwz_f21 - fwz_f40
# WEEK2 additional microstructure factors
# =========================================================

def _fwzf_ret(k):
    return _mid() / _mid().shift(k).over("SecurityID") - 1.0


def _fwzf_depth_imb10():
    return _imb(_vol("bid", 1, 10), _vol("ask", 1, 10))


def _fwzf_microprice_dev():
    micro = (
        _p("ask", 1) * _v("bid", 1)
        + _p("bid", 1) * _v("ask", 1)
    ) / (_v("bid", 1) + _v("ask", 1) + EPS)
    return _safe_div(micro - _mid(), _spread())


def _fwzf_weighted_book_pressure():
    bid = None
    ask = None
    for i in range(1, 11):
        wb = _v("bid", i) / (i * ((_mid() - _p("bid", i)).abs() + _spread() + EPS))
        wa = _v("ask", i) / (i * ((_p("ask", i) - _mid()).abs() + _spread() + EPS))
        bid = wb if bid is None else bid + wb
        ask = wa if ask is None else ask + wa
    return _imb(bid, ask)


def fwz_f21_depth_weighted_price_pressure():
    fac = _fwzf_weighted_book_pressure()
    return fac.alias("fwz_f21_depth_weighted_price_pressure")


def fwz_f22_top_depth_decay_imb():
    bid = None
    ask = None
    for i in range(1, 6):
        wb = _v("bid", i) / i
        ...
        ask = wa if ask is None else ask + wa
    fac = _imb(bid, ask)
    return fac.alias("fwz_f22_top_depth_decay_imb")


def fwz_f23_far_depth_absorption_imb():
    far_imb = _imb(_vol("bid", 6, 10), _vol("ask", 6, 10))
    trade_pressure = _trade_imbalance_expr(20)
    fac = far_imb * trade_pressure.abs()
    return fac.alias("fwz_f23_far_depth_absorption_imb")


def fwz_f24_bid_ask_depth_entropy_gap():
    bid_total = _vol("bid", 1, 10)
    ask_total = _vol("ask", 1, 10)

    bid_ent = None
    ask_ent = None

    for i in range(1, 11):
        pb = _safe_div(_v("bid", i), bid_total)
        pa = _safe_div(_v("ask", i), ask_total)

        eb = -pb * (pb + EPS).log()
        ea = -pa * (pa + EPS).log()

        bid_ent = eb if bid_ent is None else bid_ent + eb
        ask_ent = ea if ask_ent is None else ask_ent + ea

    fac = bid_ent - ask_ent
    return fac.alias("fwz_f24_bid_ask_depth_entropy_gap")


def fwz_f25_orderbook_liquidity_hole():
    bid_top = _v("bid", 1)
    ask_top = _v("ask", 1)
    bid_near = _vol("bid", 2, 5)
    ask_near = _vol("ask", 2, 5)

    bid_hole = _safe_div(bid_near, bid_top)
    ask_hole = _safe_div(ask_near, ask_top)

    fac = bid_hole - ask_hole
    return fac.alias("fwz_f25_orderbook_liquidity_hole")


def fwz_f26_spread_depth_resilience():
    depth = _vol("bid", 1, 10) + _vol("ask", 1, 10)
    fac = _safe_div(_log1p_pos(depth), _rel_spread())
    return fac.alias("fwz_f26_spread_depth_resilience")


def fwz_f27_midprice_pressure_adjusted():
    fac = _fwzf_microprice_dev() * _fwzf_depth_imb10()
    return fac.alias("fwz_f27_midprice_pressure_adjusted")


def fwz_f28_microprice_spread_interaction():
    fac = _fwzf_microprice_dev() * _rel_spread()
    return fac.alias("fwz_f28_microprice_spread_interaction")


def fwz_f29_queue_pressure_acceleration():
    qimb = _imb(_v("bid", 1), _v("ask", 1))
    fac = qimb - qimb.shift(5).over("SecurityID")
    return fac.alias("fwz_f29_queue_pressure_acceleration")


def fwz_f30_depth_imbalance_persistence():
    depth_imb = _fwzf_depth_imb10()
    fac = depth_imb.rolling_mean(window_size=20, min_periods=5).over("SecurityID")
    return fac.alias("fwz_f30_depth_imbalance_persistence")


def fwz_f31_trade_order_pressure_gap():
    fac = _trade_imbalance_expr(20) - _order_imbalance_expr(20)
    return fac.alias("fwz_f31_trade_order_pressure_gap")


def fwz_f32_trade_cancel_pressure_gap():
    fac = _trade_imbalance_expr(20) - _cancel_pressure_expr(20)
    return fac.alias("fwz_f32_trade_cancel_pressure_gap")


def fwz_f33_order_cancel_asymmetry():
    fac = _order_imbalance_expr(20) - _cancel_pressure_expr(20)
    return fac.alias("fwz_f33_order_cancel_asymmetry")


def fwz_f34_active_trade_intensity_pressure():
    intensity = _log1p_pos(_roll("trade_count", 20))
    fac = _trade_imbalance_expr(20) * intensity
    return fac.alias("fwz_f34_active_trade_intensity_pressure")


def fwz_f35_large_trade_depth_pressure():
    large_pressure = _roll_imbalance("large_buy_trade_volume", "large_sell_trade_volume", 20)
    fac = large_pressure * _fwzf_depth_imb10()
    return fac.alias("fwz_f35_large_trade_depth_pressure")


def fwz_f36_aggressive_order_depth_pressure():
    aggr_pressure = _roll_imbalance(
        "aggressive_buy_order_volume",
        "aggressive_sell_order_volume",
        20,
    )
    fac = aggr_pressure * _fwzf_depth_imb10()
    return fac.alias("fwz_f36_aggressive_order_depth_pressure")


def fwz_f37_cancel_near_far_pressure():
    near_cancel = _near_cancel_pressure_expr(20)
    far_cancel = _cancel_pressure_expr(20)
    fac = near_cancel - far_cancel
    return fac.alias("fwz_f37_cancel_near_far_pressure")


def fwz_f38_buy_cancel_risk_gate():
    buy_cancel_ratio = _safe_div(
        _roll("cancel_buy_volume", 20),
        _roll("activate_buy_order_volume", 20),
    )
    bid_conc = _safe_div(_v("bid", 1), _vol("bid", 1, 5))
    fac = -buy_cancel_ratio * bid_conc
    return fac.alias("fwz_f38_buy_cancel_risk_gate")


def fwz_f39_sell_cancel_risk_gate():
    sell_cancel_ratio = _safe_div(
        _roll("cancel_sell_volume", 20),
        _roll("activate_sell_order_volume", 20),
    )
    ask_conc = _safe_div(_v("ask", 1), _vol("ask", 1, 5))
    fac = sell_cancel_ratio * ask_conc
    return fac.alias("fwz_f39_sell_cancel_risk_gate")


def fwz_f40_flow_book_pressure_interaction():
    flow_pressure = (
        0.5 * _trade_imbalance_expr(20)
        + 0.3 * _order_imbalance_expr(20)
        + 0.2 * _cancel_pressure_expr(20)
    )
    fac = flow_pressure * _fwzf_depth_imb10()
    return fac.alias("fwz_f40_flow_book_pressure_interaction")
