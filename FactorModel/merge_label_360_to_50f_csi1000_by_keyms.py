...
FACTOR_IN = f"{ROOT}/FactorModel/data/raw/factor_features_50f_20241022_20250114_csi1000.csv"
FACTOR_OUT = f"{ROOT}/FactorModel/data/raw/factor_features_50f_20241022_20250114_csi1000_h360.csv"
LABEL_DIR = f"{ROOT}/FactorModel/data/raw/label_360_csi1000_by_date"
...
def make_key_ms(x):
    ...
    # label format usually: 2024-10-22 09:32:40.000
    ...
    # factor format usually: 20241022_093240000
...
def load_label(date):
    path = f"{LABEL_DIR}/label_360_{date}_csi1000.csv"
    ...
    if "datetime" in cols:
        ...
        raise RuntimeError(f"cannot find time key column in {path}: {cols}")
...
def main():
    ...
    for chunk in pd.read_csv(
        ...
        if total <= CHUNKSIZE and non_na == 0:
            raise RuntimeError("first chunk label_360 is all NaN; merge key not matched")
