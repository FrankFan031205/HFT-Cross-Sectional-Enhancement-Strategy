...
DL1_LOG="${FACTOR_DIR}/logs/train_cs_dl_factors_20_${TAG}_gpu.log"
DL2_LOG="${FACTOR_DIR}/logs/train_cs_dl_factors_20_v2_${TAG}_gpu.log"
...
wait_for_dump() {
    echo
    echo "================================================================================"
    echo "Waiting for 742 factor dump to finish"
    ...
    while true; do
        ...
        if pgrep -f "dump_factor_features_fixed_universe.py" >/dev/null 2>&1; then
            ...
            echo "ERROR: dump stopped but input file does not exist"
            if [[ -f "${DUMP_LOG}" ]]; then
...
if len(fwz_cols) == 0:
    raise SystemExit("ERROR: no fwz factor columns found")
...
summarize_eval "${DL1_EVAL}" "DL BATCH1 TEST RANKING"
summarize_eval "${DL2_EVAL}" "DL BATCH2 TEST RANKING"
