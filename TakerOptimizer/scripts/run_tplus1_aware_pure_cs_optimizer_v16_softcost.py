...
def normalize_market_input(df: pd.DataFrame, signal_col: str) -> pd.DataFrame:
    ...
    soft_col = pick_col(
        df,
        ["soft_cost_signal", "pred_ret_h10", "pred_ret_10", "pred_h10", "alpha_h10", "signal_h10"],
        required=False,
        name="soft cost signal",
    )
    ...
    out["soft_cost_signal"] = (
        df[soft_col].astype(float).values
...
def solve_one_rebalance(cur: pd.DataFrame, state: dict, args, daily_turnover_used: float):
    ...
    # ===== V16 SOFTCOST PATCH START =====
    soft_raw = cur["soft_cost_signal"].values if "soft_cost_signal" in cur.columns else np.zeros(n, dtype=float)
    soft_trade_signal = zscore_signal(soft_raw, float(args.soft_cost_signal_clip)) * float(args.soft_cost_scale)

    mid_px = cur["mid_price"].values.astype(float)
    bid_px = cur["bid_price"].values.astype(float)
    ask_px = cur["ask_price"].values.astype(float)
    mid_safe = np.where(mid_px > 0, mid_px, np.maximum((bid_px + ask_px) / 2.0, 1e-12))
    fee_rate_for_obj = float(args.fee_bps) / 10000.0

    buy_exec_cost = np.zeros(n, dtype=float)
    sell_exec_cost = np.zeros(n, dtype=float)
    if int(getattr(args, "use_execution_cost_in_objective", 1)) == 1:
        buy_exec_cost = np.maximum(ask_px / mid_safe - 1.0, 0.0) + fee_rate_for_obj
        sell_exec_cost = np.maximum(1.0 - bid_px / mid_safe, 0.0) + fee_rate_for_obj

    base_buy_cost = float(args.lambda_turnover) + buy_exec_cost
    ...
    obj = (
        signal @ w
        - cp.sum(cp.multiply(buy_cost, buy))
        - cp.sum(cp.multiply(sell_cost, sell))
    ...
    return {
        ...
        "avg_buy_cost": float(np.mean(buy_cost)),
        "avg_sell_cost": float(np.mean(sell_cost)),
        "avg_soft_trade_signal": float(np.mean(soft_trade_signal)),
        "std_soft_trade_signal": float(np.std(soft_trade_signal)),
...
def main():
    ...
    ap.add_argument("--soft-cost-signal-clip", type=float, default=5.0)
    ap.add_argument("--use-execution-cost-in-objective", type=int, default=1)
    ...
    for (date, dt), cur in df.groupby(["date", "datetime"], sort=True):
        ...
        summary_rows.append({
            ...
            "avg_buy_cost": sol.get("avg_buy_cost", np.nan),
            "avg_sell_cost": sol.get("avg_sell_cost", np.nan),
            "avg_soft_trade_signal": sol.get("avg_soft_trade_signal", np.nan),
            "std_soft_trade_signal": sol.get("std_soft_trade_signal", np.nan),
