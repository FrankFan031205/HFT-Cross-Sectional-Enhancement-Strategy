...
def is_rebalance_minute(minute, ocfg):
    interval = int(ocfg.get("portfolio_rebalance_interval_min", 1))
    print("[DEBUG] portfolio_rebalance_interval =", interval, "raw_ocfg =", ocfg.get("portfolio_rebalance_interval_min", None), flush=True)
    if interval <= 1:
        return True
...
def compute_desired_weights(df_t, state_qty, state_weight, state_entry_i, state_trade_i, minute_i, cfg):
    ...
    target_gross = float(ocfg.get("target_gross_limit", 0.60))
    ...
    for _, row in df_t.iterrows():
        ...
        if not trade_allowed or not rebalance_allowed:
            ...
            if current_qty <= 0:
                if is_global_top and cooldown_left == 0:
                    dw = per_name_weight
                    st = "ENTRY"
                else:
                    dw = 0.0
